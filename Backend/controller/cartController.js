...
const addToCart = async (request, response) => {
  try {
    ...
    response.json({
      success: true,
      msg: "product is successfully add to your cart",
    });
  } catch (error) {
    console.log(error);
...
const updateCart = async (request, response) => {
  try {
    ...
    return response
      .status(200)
      .json({ success: true, msg: "Cart updated successfully" });
  } catch (error) {
    console.log(error);
    response.json({ success: false, msg: error.message });
  }
...
const getCartData = async (request, response) => {
  try {
    ...
    response.status(500).json({
      success: false,
      msg: "An error occurred while fetching cart data",
    });
  }
};
...
const clearCart = async (request, response) => {
  try {
    ...
    response.json({ success: true, message: "Cart cleared successfully" });
  } catch (error) {
...
export { addToCart, updateCart, getCartData, clearCart };
