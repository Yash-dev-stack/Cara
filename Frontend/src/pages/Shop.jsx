...
const Shop = () => {
  ...
  const applyFilter = () => {
    ...
    if(category.length> 0) {
      productCopy = productCopy.filter(elem => category.includes(elem.category))
    } 
    if(subCategory.length > 0) {
      productCopy = productCopy.filter(elem => subCategory.includes(elem.subCategory))
    }
  ...
  return (
    ...
    <h1 className="text-[40px] font-extrabold leading-[54px] text-white ">#Shop Now</h1>
    <p className="text-white font-medium">Save more with coupons & upto 70% off</p>
    </div>
    ...
    {/*Category*/}
      ...
      <p className='flex gap-2 items-center'>
       <input onChange={toggleCategory} type='checkbox' value={'Mens'} /> Mens
      </p>
      <p className='flex gap-2 items-center'>
       <input onChange={toggleCategory} type='checkbox' value={'Womens'} /> Womens
      </p>
      <p className='flex gap-2 items-center'>
    ...
    <div className='flex items-center justify-between flex-row'>
     ...
     <select onChange={(e) => setSortType(e.target.value)} className='border-2  py-2 text-sm text-gray-400 rounded' >
   <option value='relevent'>Short by: Relevent </option>
    ...
    <div className=' grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6 px-6 py-5'>
    {
      filterProduct.map((elem, index) => (
        <Products key={elem._id || index} image={elem.image} name={elem.name} price={elem.price} id={elem._id} />
      ))
    }
    </div>
    </div>
